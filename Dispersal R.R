...
data.predict<- read.table("Pasta.csv", sep = ";", h = T, dec = ".")
...
dispeRsal(data.predict, model = 5, CI = FALSE, random = TRUE,
          tax = "family")
...
data.predict$Species <- str_replace_all(data.predict$Species, "\\.", "_")

data.predict <- data.predict |> mutate(GF = as.factor(GF),
                                       DS = as.factor(DS))

test<-dispeRsal(data.predict, model = 5, CI = T, random = F,
                tax = "family")
test

write.csv(test, "Caatinga.csv")

View(test)

write.csv(test["predictions"], "MDDCaatinga.csv")
